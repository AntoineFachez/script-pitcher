...
def process_shapes_and_lines(page, z_counter):
    ...
    for path in drawings:
        ...
        # Juri's Fix: We MUST keep potential cover shapes (large white/background rectangles)
        # as they are used by Mac apps to obscure/crop the underlying full image.
        # This filter is removed to allow these crucial vector elements to be processed.
        # Original Filter has been commented out to keep the element:
...
def process_images(doc, page, image_bucket, document_id, z_counter):
    ...
    for img_index, info in enumerate(image_info_list):
        ...
        try:
            ...
            # --- 4. Upload Logic ---
            # Update the image path to use the fileId as the base directory and a consistent subdirectory for extracted images.
            # The format is: [fileId]/page_images/page_N_block_I.ext
            image_filename = f"{document_id}/page_images/page{page.number + 1}_img{img_index}.{image_ext}"
