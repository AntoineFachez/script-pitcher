...
describe("POST /api/characters", () => {
  ...
  it("should return 400 if projectId is missing", async () => {
    ...
    expect(body.error).toBe("Project ID is required to create a character.");
  });
  ...
  it("should successfully create a character if user is an owner", async () => {
    ...
    const req = mockRequest({ projectId, ...characterData }, "valid-token");
    const response = await POST(req);

    // Assertions
    expect(response.status).toBe(201);
    const body = await response.json();
    expect(body.id).toBe("new-char-id");
  ...
  it("should return 500 on a server error during creation", async () => {
    ...
    const body = await response.json();
    expect(body.error).toBe("Failed to create character.");
  });
});
